...
# 第二个工作流 — 技能自动匹配
#
# skills="*" 会让框架根据用户输入自动匹配最相关的技能
# 匹配基于技能名称和描述的相似度计算
# ============================================================
workflow_auto = Workflow(
    id="smart_agent",
    name="06B Smart Agent",
    description="技能自动匹配：根据用户输入自动选择最相关的技能。",
    welcome="🧠 我会根据你的问题自动匹配相关技能来帮你。",
    inputs=[
        Input("user_input", str, required=True, description="User request"),
    ],
    user_input="user_input",
...
workflow_auto.add_node(LLMNode(
    id="agent",
    system_prompt="You are a versatile assistant that can help with various tasks.",
    skills="*",
    stream=True,
    output_to_user=True,
))
...
async def main():
    ...
    # 自动匹配
    print("\n--- Auto-matching Skill ---")
    result = await workflow_auto.run({
        "user_input": "I need to make an animated GIF for Slack"
    })
    print(f"Response: {result['state'].get('agent', '')[:500]}...")


workflow.publish()
workflow_auto.publish()
