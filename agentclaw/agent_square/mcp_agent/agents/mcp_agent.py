...
Use service-published MCP tools from this template's mcps/example_tools.py.
...
def _load_template_mcp_tools() -> None:
    tools_path = Path(__file__).resolve().parents[1] / "mcps" / "example_tools.py"
    if not tools_path.is_file():
        return
    module_name = f"{__name__}_example_tools"
