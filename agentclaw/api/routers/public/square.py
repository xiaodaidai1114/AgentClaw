...
from agentclaw.api.routers.public.access import is_builtin_workflow, is_public_share_enabled
...
def _is_square_published(workflow: Any, workflow_id: str) -> bool:
    return (
        ...
        and not is_builtin_workflow(workflow, workflow_id)
...
def _square_workflow_payload(workflow: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "share_token": str(getattr(workflow, "public_share_token", "") or "").strip(),
        "chat_audio": {
