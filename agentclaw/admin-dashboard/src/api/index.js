...
const publicShareHeaders = (shareToken = '', extraHeaders = {}) => {
  const headers = { ...publicSessionHeaders, ...extraHeaders }
  if (shareToken) headers['X-AgentClaw-Share-Token'] = shareToken
  return headers
}
...
export const publicWorkflowsApi = {
  get: (workflowId, shareToken = '') => publicApi.get(`/public/workflows/${encodeURIComponent(workflowId)}`, { headers: publicShareHeaders(shareToken), withCredentials: true }),
...
export const publicAudioApi = {
  speechToText: (workflowId, shareToken = '', file) => {
    ...
    return publicApi.post(`/public/workflows/${encodeURIComponent(workflowId)}/speech-to-text`, form, {
      headers: publicShareHeaders(shareToken, { 'Content-Type': 'multipart/form-data' }),
      withCredentials: true,
    })
  },
...
export const publicConversationsApi = {
  ...
  delete: (workflowId, conversationId, shareToken = '') => publicApi.delete(`/conversations/${encodeURIComponent(workflowId)}/${encodeURIComponent(conversationId)}`, { headers: publicShareHeaders(shareToken), withCredentials: true }),
  // 反馈 API
  submitFeedback: (workflowId, conversationId, messageIndex, feedback, shareToken = '') =>
    publicApi.post(`/conversations/${encodeURIComponent(workflowId)}/${encodeURIComponent(conversationId)}/feedback`, { message_index: messageIndex, feedback }, { headers: publicShareHeaders(shareToken), withCredentials: true }),
  getFeedback: (workflowId, conversationId, shareToken = '') =>
    publicApi.get(`/conversations/${encodeURIComponent(workflowId)}/${encodeURIComponent(conversationId)}/feedback`, { headers: publicShareHeaders(shareToken), withCredentials: true }),
}
