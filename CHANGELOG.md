...
- 公开智能体分享页和公开房间链接新增模型切换能力，页面会通过公开工作流/房间模型列表接口加载当前工作流可用的对话模型；接口仍按公开访问权限校验 `share_token`、广场发布状态或房间令牌，不复用 Admin 模型列表。

**Changed**

- 项目版本从 `1.1.6` 更新为 `1.1.7`，同步 `VERSION`、运行时 fallback 版本、Python 包元数据、`uv.lock`、Dashboard package 元数据和 README 徽章。
- 海龟汤回合上下文把稳定参考题库放在前面，本轮 session 与用户输入放在后面，减少提示词缓存失效。

**Fixed**

- 修复海龟汤一轮结束后开启下一轮时，如果模型返回的 session 仍残留旧汤底、事实、计数或历史，下一轮可能受上一轮影响的问题；现在 solved 后重开会确定性清理旧状态。
- 修复公开多人会话运行期间轮询返回较旧历史时，可能覆盖本地已经显示的玩家乐观消息，导致发送后历史消息短暂消失的问题。
- 修复历史用户消息点击编辑后，编辑框继承紧凑气泡宽度而被压成很窄的问题。
- 修复历史用户消息编辑重发时 Admin checkpoint 截断多保留被编辑旧输入的问题；同时修复公开智能体页面没有 Admin token 时编辑重发会直接中断、不再触发工作流的问题。
- 修复公开智能体链接和公开房间链接没有模型切换的问题；公开运行请求现在会携带并校验本次选择的对话模型，且不会暴露或接受 embedding、rerank、语音识别、语音合成等非对话模型作为切换目标。
- 修复公开页面模型切换按钮与主对话页面样式不一致的问题，桌面端公开头部现在复用主对话页的模型切换按钮结构和下拉定位。

**Tests**

- 新增/扩展海龟汤下一轮状态清理、提示词上下文顺序、公开房间乐观消息保留、历史消息编辑框宽度、Admin 编辑重发 checkpoint 截断、公开智能体编辑重发、公开工作流/房间模型列表权限与公开模型切换回归测试，并重新构建 Dashboard `dist` 产物。

## [1.1.6] - 2026-06-08

**Added**

- 公开多人会话新增有效期选择，创建房间时可选择 1 天、7 天或 30 天，最长 30 天；旧房间会按创建时间回填 30 天有效期。
- Dashboard 新增“公开会话”管理页和 Admin API，可查看公开房间列表、房间详情、成员、智能体对话、玩家聊天，并支持删除房间与踢出成员。

**Changed**

- 项目版本从 `1.1.5` 更新为 `1.1.6`，同步 `VERSION`、运行时 fallback 版本、Python 包元数据、`uv.lock`、Dashboard package 元数据和 README 徽章。
- 公开多人会话的用户输入会在送入工作流时附带玩家昵称，便于模型区分不同玩家；页面中保存和展示的用户消息仍保留原始正文。

**Fixed**

- 修复同一个公开房间内不同用户可以使用重复昵称的问题；同一成员可用原昵称重连，其他成员占用昵称时会返回明确冲突错误。
- 修复公开房间无限期保留、缺少后台查看和清理入口的问题，过期或已撤销房间不再作为公开入口访问。

**Tests**

- 新增/扩展公开房间昵称冲突、用户输入昵称前缀、房间有效期、过期访问、Admin 公开房间管理接口和 Dashboard 公开会话管理页回归测试，并重新构建 Dashboard `dist` 产物。
